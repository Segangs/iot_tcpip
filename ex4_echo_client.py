...
def main():
    ...
    while True:
        message = input("Input message (q to exit)") #Printf와 scanf를 동시에 함
        if message.lower() == 'q' :
            break
        sock.send(message.encode('utf-8'))
        recerived_data = sock.recv(1024) # 서버에서 오는 메세지를 받음
        if not recerived_data:
            error_handling("socket read() error")
        print(f"message from server: {recerived_data.decode("utf-8")}")
